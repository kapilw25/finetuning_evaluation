...
# No unsloth or quantization needed for BF16 evaluation
...
def load_model_with_adapter(adapter_repo):
    """Load base model in BF16 (no quantization) and apply LoRA adapter from HuggingFace"""
    print(f"  📦 Loading base model: {BASE_MODEL_NAME} (BF16, no quantization)")
    ...
    # Load base model in full BFloat16 precision (no quantization)
    ...
    # Set Llama-3 chat template (required for CITA models trained with custom template)
    # This matches the template used during training (see CITA_Baseline/Llama3_BF16.ipynb cell 10)
    ...
    if adapter_repo is None:
        ...
        print(f"  🔧 Loading BF16 LoRA adapter from HuggingFace: {adapter_repo}")
...
def evaluate_model(model_name: str, adapter_repo) -> Dict:
...
def main():
    ...
    # Evaluate all models (loaded from HuggingFace)
    all_results = {}
    for model_name, adapter_repo in MODELS.items():
        result = evaluate_model(model_name, adapter_repo)
