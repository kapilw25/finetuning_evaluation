...
from src.utils.eval.plotting import (
...
def main():
    ...
    for eval_name, eval_dir in EVAL_DIRS.items():
        ...
        if len(deltas) >= 2:
            ...
            print("  [WARN] Not enough methods with both variants for radar chart")
    ...
    if has_any_ci:
        ...
        print("       To enable CI, re-run evaluations with per_sample_scores in metrics.json")
    ...
    if len(eval_scores) >= 2:
        ...
        print("  [OK] heatmap_no_ci.{pdf,png}")
    else:
    ...
    if generated['heatmap']:
        print("  - heatmap.{pdf,png} (with CI)")
    if generated['heatmap_no_ci']:
        print("  - heatmap_no_ci.{pdf,png} (without CI)")
