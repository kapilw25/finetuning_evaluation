"""PPO training on PKU-SafeRLHF (reward-model + value-head, init from SFT LoRA). GPU-only.

Usage (6 commands = 3 modes × 2 instruction settings):

    # MICRO: 0.05 epochs (~1 hour) - quick pipeline validation
    python -u src/train/ppo.py --mode micro --use-instruction false --no-push 2>&1 | tee logs/ppo_micro_NI.log
    python -u src/train/ppo.py --mode micro --use-instruction true  --no-push 2>&1 | tee logs/ppo_micro_I.log

    # SANITY: 0.3 epochs (~5 hours) - validate checkpoints, --no-push for safe local-only test
    python -u src/train/ppo.py --mode sanity --use-instruction false --no-push 2>&1 | tee logs/ppo_sanity_NI.log
    python -u src/train/ppo.py --mode sanity --use-instruction true  --no-push 2>&1 | tee logs/ppo_sanity_I.log

    # FULL: 1.0 epoch (~17 hours) - production training - ALERT - use TMUX terminal to avoid INTERRUPTION
    TMUX >> python -u src/train/ppo.py --mode full --use-instruction false 2>&1 | tee logs/ppo_full_NI.log
    TMUX >> python -u src/train/ppo.py --mode full --use-instruction true  2>&1 | tee logs/ppo_full_I.log
...
# ===================================================================
from src.utils.train.model_utils import (
    load_hf_token,
    get_model_repo_name,
    get_test_prompts,
...
from src.utils.train.data_prep.loader_pku import load_pku_combined_clear_contrast, get_safe_unsafe_responses, synthesize_system_instruction
from src.utils.train.push_automation import PushAutomation
from src.utils.logging_utils import setup_training_logger, restore_logging
from src.utils.train.monitoring_callback import print_ppo_summary
...
def prepare_ppo_dataset(tokenizer, use_instruction: bool = False, max_samples: int = None):
    ...
    if use_instruction:
        print("   Using CUSTOMIZED instructions per example (fair comparison with DPO)")
...
def train_ppo_baseline(
    ...
    print("  - Method: Proximal Policy Optimization (Schulman 2017)")
    print(f"  - Training epochs: {num_epochs}")
    print("  - Learning rate: 1e-5 (Meta's Llama 3 setting)")
    ...
    if force_skip:
        ...
        if latest_checkpoint:
            ...
            print("   Will start fresh training (even if HF repo exists)...\n")
            latest_checkpoint = None
    ...
    if not training_skipped:
        ...
        if base_model:
            ...
            if latest_checkpoint and resume_step > 0:
                ...
                try:
                    ...
                    print("   Falling back to fresh PPO LoRA init...")
                    resume_step = 0
                    latest_checkpoint = None
        ...
        if latest_checkpoint and is_training_complete(latest_checkpoint, total_steps):
            ...
            print(f"   Total steps: {total_steps} ({num_epochs} epochs)")
            print("   Skipping training, will load from local checkpoint for inference...\n")
            training_skipped = True
            load_from_hf = False
  ...
  python comparative_study/02b_PPO_Baseline/Llama3_BF16.py --epochs 0.5 --use-instruction false
    ...
    try:
        ...
        if repo_exists(HF_REPO, token=HF_TOKEN, repo_type="model"):
            ...
            print("   This will be the first training run")
    except Exception as e:
    ...
    print("\nOptions:")
    if hf_model_exists:
        print("  1) Inference only from HF_repo (use existing HF model)")
        print("  2) Retrain and replace HF model (regardless of performance)")
    else:
        print("  1) Inference only from HF_repo (UNAVAILABLE - no model)")
