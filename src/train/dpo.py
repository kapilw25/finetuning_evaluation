...
Usage (4 commands = 2 modes × 2 instruction settings):
  ...
  | Mode                | Steps | ETA       |
  |---------------------|-------|-----------|
  | Sanity (0.3 epochs) | ~102  | ~31 min   |
  | Full (1.0 epoch)    | ~338  | ~103 min  |
"""
...
from src.utils.train.data_prep.formatters import format_pku_for_dpo_NoInstruct, format_pku_for_dpo_Instruct
...
def train_dpo_baseline(num_epochs=1.0, output_dir=None, base_model=None, force_skip=False):
    ...
    print("  - Method: Standard DPO (Rafailov 2023)")
    print("  - Loss: L_DPO only")
    print(f"  - Training epochs: {num_epochs}")
    print("  - Batch size: 1 (per device, reduced for DPO ref model)")
    print("  - Gradient accumulation: 8 (effective batch=8)")
    print("  - Learning rate: 1e-5 (Meta's Llama 3 DPO setting)")
    print("  - Beta: 0.1 (Meta's Llama 3 DPO setting)")
    ...
    if force_skip:
        ...
        if latest_checkpoint:
            ...
            print("   Will check completion status after loading dataset...\n")
        else:
