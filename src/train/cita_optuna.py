...
# ===================================================================
from src.utils.train.model_utils import load_hf_token
...
def train_cita_trial(trial, max_steps=200, base_model=None, use_instruction=False, run_name="CITA_Adaptive"):
    ...
    print("  - Callbacks: TrainingSummaryCallback only (matches DPO baseline)")
    print(f"{'='*80}\n")

    # ===== LOAD MODEL & TOKENIZER =====
    from src.utils.train.model_utils import load_model_bf16
    ...
    # ===== APPLY LORA ADAPTERS =====
    from src.utils.train.model_utils import setup_lora
    ...
    # from src.utils.train.model_utils import apply_torch_compile
    # model = apply_torch_compile(model)

    # ===== LOAD DATASET =====
    from src.utils.train.data_prep.loader_pku import load_pku_combined_clear_contrast
    from src.utils.train.data_prep.formatters import format_pku_for_cita_Instruct, format_pku_for_cita_NoInstruct
    ...
    from src.utils.train.monitoring_callback import TrainingSummaryCallback
    ...
    # ===== CREATE CITA TRAINER =====
    from src.utils.train.cita_trainer import CITATrainer
    ...
    if max_grad_norm is None or max_grad_norm == 0:
        ...
        print("   Finding stable hyperparameters WITHOUT clipping band-aid")
        print("   Explosion detector will prune unstable trials (grad_norm > 50.0)")
        print("   This finds HPs that are truly stable, not just clipping-masked\n")
    else:
        print(f"\n✅ Gradient clipping ENABLED: max_grad_norm={max_grad_norm}")
        print("   WARNING: Clipping may mask unstable HPs!")
        print("   Monitor 'cita/grad_norm_clipped' and 'cita/clipping_active' in logs\n")
    ...
    try:
        ...
        if "exploded" in str(e).lower() or "grad_norm" in str(e).lower():
            ...
            print(f"   Error: {e}")
            print("   Hyperparameters caused training instability")
            ...
            if hasattr(trainer, 'state') and hasattr(trainer.state, 'log_history'):
                for log in reversed(trainer.state.log_history):
                    if 'eval_rewards/margins' in log:
                        ...
                        print("   (Intermediate reporting disabled for multi-objective optimization)")
                        reported = True
                        break

            if not reported:
                print("   No eval metrics found (explosion before first eval)")

            print("   Cleaning up and marking trial as pruned\n")
    ...
    try:
        ...
        print("   Metrics still recorded - Optuna will continue\n")
...
def run_optuna_cita_search(
    ...
    if db_exists:
        print(f"\n{'='*80}")
        print("📊 EXISTING OPTUNA DATABASE FOUND")
        print(f"{'='*80}")
        print(f"Location: {storage_path}")
        print("\nOptions:")
        print("  1) Fresh run (delete previous trials, start from scratch)")
        print("  2) Continue from previous run (load existing trials)")
        print(f"{'='*80}\n")
        ...
        if choice == "1":
            ...
            print("✅ Database deleted. Starting fresh run.\n")
        elif choice == "2":
    ...
    print("Sampler: TPE (truly adaptive)")
    print("Pruner: Hyperband (min_resource=50, early stopping)")
    print("Monitoring: TrainingSummaryCallback every 50 steps")
    print(f"{'='*80}\n")
    ...
    print(f"\n{'='*80}")
    print("🏆 PARETO-OPTIMAL TRIALS FOUND")
    print(f"{'='*80}")
