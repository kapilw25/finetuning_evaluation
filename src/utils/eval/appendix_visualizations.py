...
def generate_instruction_fidelity_heatmap(
    ...
    if len(valid_models) < 2:
        print("  [SKIP] Not enough models with fidelity data")
        return []
...
def generate_length_control_distribution(
    ...
    if len(valid_models) < 1:
        print("  [SKIP] No valid models for length control viz")
        return []
    ...
    for ax, model in zip(axes, valid_models):
        ...
        # Get model color
        color = get_model_color(model)  # noqa: F841
...
def generate_truthfulqa_category_heatmap(
    ...
    if len(model_category_data) < 2:
        print("  [SKIP] Not enough models with category data")
        return []
