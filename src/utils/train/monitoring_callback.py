    ...
    def on_step_end(self, args, state, control, model=None, tokenizer=None, **kwargs):
            ...
            if current_margin is None:
                for log_entry in reversed(state.log_history):
                    if 'cita/margin' in log_entry:
                        ...
                        print("⚠️  Note: Using TRAIN margin (eval margin not available yet)")
                        break
        ...
        if current_margin is not None:
            ...
            if current_margin <= self.margin_tolerance:
                ...
                print("   Model prefers REJECTED (unsafe) responses!")
                print("   🛑 STOPPING IMMEDIATELY (no tolerance for unsafe behavior)")
        ...
        if current_kl is not None:
            ...
            if current_kl >= self.kl_threshold:
                ...
                print("   Model drifting from reference (mode collapse risk)!")
                print("   🛑 STOPPING IMMEDIATELY (preventive)")
        ...
        if unsafe_behavior_detected and self.stop_on_negative_margin:
            failure_reasons.append("NEGATIVE MARGIN (model prefers unsafe responses)")

        # Check 2: KL drift (preventive)
        if kl_drift_detected and self.stop_on_high_kl:
            failure_reasons.append("KL DIVERGENCE TOO HIGH (drift from reference)")

        # Check 3: Gibberish (mode collapse)
        if gibberish_detected and self.stop_on_gibberish:
            failure_reasons.append("GIBBERISH DETECTED (mode collapse)")
...
class TrainingSummaryCallback(TrainerCallback):
    ...
    def _print_dpo_summary(self, recent_logs, state):
        ...
        if margins:
            ...
            if negative_count > len(margins) * 0.1:
                print("  ⚠️  WARNING: >10% negative margins = model prefers UNSAFE responses!")
        ...
        if accuracies:
            ...
            if avg_acc < 80:
                print("  ⚠️  WARNING: Accuracy < 80% = model learning WRONG preferences!")
    ...
    def _print_cita_summary(self, recent_logs, state):
        ...
        if total_losses:
            ...
            if loss_kl:
                ...
                if avg_kl < -2.0:
                    print("  ⚠️  WARNING: High negative KL = model drifting from reference!")
        ...
        if margins:
            ...
            if negative_count > len(margins) * 0.1:
                ...
                print("     Model systematically prefers UNSAFE responses!")
        ...
        if accuracies:
            ...
            if avg_acc < 60:
                print("  🚨 CRITICAL: Accuracy < 60% = worse than random guessing!")
            elif avg_acc < 80:
                print("  ⚠️  WARNING: Accuracy < 80% = model learning WRONG preferences!")
        ...
        if eval_margins or eval_accs:
            ...
            if eval_accs:
                ...
                if val_acc < 70:
                    print("  🚨 CRITICAL: Validation accuracy < 70%!")
    ...
    def _print_grpo_summary(self, recent_logs, state):
        ...
        if rewards_mean:
            ...
            if len(rewards_mean) > 10:
                ...
                if second_half > first_half:
                    ...
                    print("  ⚠️  WARNING: Reward declining - model may be learning wrong behavior!")
        ...
        if kl_values:
            ...
            if avg_kl > 0.5:
                print("  ⚠️  WARNING: KL > 0.5 = significant drift from reference model!")
            elif avg_kl > 1.0:
                print("  🚨 CRITICAL: KL > 1.0 = severe drift, risk of mode collapse!")
        ...
        if completion_lengths:
            ...
            if avg_len < 10:
                print("  ⚠️  WARNING: Very short completions - possible reward hacking!")
            elif avg_len > 500:
                print("  ⚠️  WARNING: Very long completions - possible verbosity issue!")

        # Summary status
        if rewards_mean and kl_values:
            print("\nSTATUS:")
            if rewards_mean[-1] > 0 and kl_values[-1] < 0.5:
                print("  ✅ HEALTHY: Positive reward with controlled KL")
            elif rewards_mean[-1] <= 0:
                print("  ⚠️  CONCERNING: Negative/zero reward")
            elif kl_values[-1] >= 0.5:
                print("  ⚠️  CONCERNING: High KL divergence")
...
def print_ppo_summary(
    ...
    if recent_kl:
        ...
        if avg_kl > 0.5:
            print("  ⚠️  WARNING: KL > 0.5 = significant drift from reference!")
        elif avg_kl > 1.0:
            print("  🚨 CRITICAL: KL > 1.0 = severe drift, risk of mode collapse!")
