...
def load_model_bf16(
    ...
    if use_flash_attention:
        ...
        print("✅ Flash Attention 2 enabled (saves ~2.5GB memory)")
...
def apply_torch_compile(model):
    ...
    try:
        ...
        print("✅ Disabled CUDAGraph for dynamic shapes (prevents eval OOM)")
        ...
        print("✅ Disabled quantization fusion (fixes PyTorch 2.5.1 inductor bug)")

        # Compile model (10-20% speedup expected)
        model = torch.compile(model, mode="reduce-overhead")
        print("✅ torch.compile() enabled (expect 10-20% speedup)")
