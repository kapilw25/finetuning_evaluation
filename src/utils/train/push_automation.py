...
class PushAutomation:
    ...
    def _configure_git(self):
        ...
        try:
            ...
            print("   Continuing anyway (git may already be configured)")
    ...
    def _get_previous_best_margin(self, hf_repo: str, metric_name: str = "eval_loss") -> Optional[float]:
        ...
        try:
            ...
            print("   Assuming this is first training run")
            return None
    ...
    def should_push_to_hf(self, current_metric: float, hf_repo: str, metric_name: str = "eval_loss", metric_mode: str = "max") -> bool:
        ...
        if previous_metric is None:
            ...
            print("   Will push to HuggingFace (establishing baseline)")
            return True
        ...
        print("   Will push to HuggingFace (overwriting previous model)")
        return True
    ...
    def save_local_backup(
        ...
        try:
            ...
            print("⚠️  Instance will shutdown - this backup is critical!")
            print(f"{'='*80}\n")

            # Load best hyperparameters
            with open(config_path, "r") as f:
                best_config = json.load(f)  # noqa: F841  # legacy: kept for validation side-effect
            ...
            print(f"\n❌ Local save failed: {e}")
            print("⚠️  WARNING: No backup will be available after instance shutdown!")
            import traceback
            traceback.print_exc()
            return None
    ...
    def push_to_huggingface(
        ...
        if hasattr(best_trial, 'last_result'):
            ...
            if metric_name == "cita/margin" and "rewards/margin" in best_trial.last_result:
                ...
                print("ℹ️  Using rewards/margin for comparison (cita/margin is log-prob scale)")
            else:
        ...
        try:
            ...
            print(f"Checkpoint: {best_checkpoint}")
            print("This will REPLACE the existing model (performance improved)")
            print(f"{'='*80}\n")
...
"""
            ...
            try:
                ...
                if repo_exists(hf_repo, token=self.hf_token, repo_type="model"):
                    ...
                    print("   ✅ Deleted existing repo")
                else:
                    print("   ℹ️  Repo doesn't exist (first push)")
            except Exception as e:
            ...
            api.upload_file(
                ...
                repo_id=hf_repo,
                token=self.hf_token,
                commit_message="Add model card with training hyperparameters"
            )
            print("✅ Uploaded model card (README.md) with training hyperparameters")
            ...
            if trainer_state_path:
                ...
                print("   Skipping metrics upload")

            print(f"\n{'='*80}")
            print("✅ LoRA adapter successfully pushed to HuggingFace!")
            print(f"{'='*80}")
            print(f"🔗 View at: https://huggingface.co/{hf_repo}")
            print(f"📊 Best hyperparameters: {config_path}")
            print("📏 Upload size: ~165MB (adapter only, not merged model)")
            print("✅ Compatible with inference_bf16.py")
            print(f"{'='*80}\n")
            ...
            print(f"{'='*80}")
            print("⚠️  Model training succeeded but push failed.")
...
"""
            ...
            if result.returncode == 0:
                ...
                print("⚠️  Push failed, trying with upstream setup...")
                subprocess.run(
...
"""
        ...
        print("  - GitHub: Logs and code pushed")
        print(f"{'='*80}\n")
