...
% =============================================================================
\begin{abstract}
Traditional LLM alignment methods like DPO, PPO, and GRPO embed static objectives into model weights, lacking flexibility for dynamic policy changes. We introduce \textbf{CITA (Contrastive Instruction-Tuned Alignment)}, a framework enabling \textbf{instruction-driven alignment} at inference time. CITA combines supervised fine-tuning, contrastive preference optimization, and mandatory KL regularization to achieve instruction-conditioned behavior switching.
...
Experiments on Llama-3.1-8B across 5 benchmarks demonstrate CITA's dominance over all baselines. Measuring instruction-alignment efficiency (how well models adapt behavior based on instructions), \textbf{CITA achieves 95.6\%}---outperforming DPO (70.5\%) by 25.1 pp, GRPO (59.3\%) by 36.3 pp, PPO (50.4\%) by 45.2 pp, and SFT (15.9\%) by 79.7 pp. On TruthfulQA, CITA shows 54$\times$ stronger adaptation than DPO (+0.054 vs +0.001). Our results demonstrate that mandatory KL regularization enables deep instruction-alignment rather than superficial instruction-following.
\end{abstract}
...
\begin{itemize}[leftmargin=*,itemsep=2pt]
    ...
    \item \textbf{Comprehensive Evaluation}: CITA achieves 95.6\% instruction-alignment efficiency, beating DPO by 25.1 pp, GRPO by 36.3 pp, PPO by 45.2 pp, and SFT by 79.7 pp
    \item \textbf{Key Insight}: Mandatory KL regularization enables deep instruction-alignment---54$\times$ stronger TruthfulQA adaptation than DPO
\end{itemize}
\end{defin}
