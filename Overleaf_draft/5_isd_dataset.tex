...
\paragraph{Prompt Bank.} We curate 500 prompts from diverse sources, following prompt curation best practices from recommendation systems~\cite{yang2023prompt}:
...
This structure enables systematic evaluation: each prompt appears exactly 10 times (once per instruction type), allowing direct comparison of behavioral responses while controlling for prompt-specific effects. The controlled factorial design follows principles from large-scale text generation datasets~\cite{roy2025comprehensive}. The 300 base prompts are drawn from 12 categories (25 prompts each), ensuring topical diversity across economics, healthcare, technology, ethics, and other domains.
