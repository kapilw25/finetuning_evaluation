...
\textbf{(ii) directional purity} (a signed difference of two conditional policy gradients under the \emph{same} $(I,X)$), isolating the effect of $I$ rather than prompt artifacts.
...
\textbf{Base $\rightarrow$ SFT} to stabilize instruction-following priors and response formatting; then \textbf{SFT $\rightarrow$ CITA} to learn the conditional preference field under a KL-shaped trust region. cf. Appendix. 

\vspace{-0.35em}
\paragraph{Takeaway.}
\textbf{CITA optimizes an instruction-indexed family of nearby optima; the KL term supplies the geometry that makes switching reliable.}
