...
\title{\includegraphics[width=\textwidth]{banner.pdf}}
...
To measure \emph{instruction-alignment} (deep policy embodiment) versus mere instruction-following, we introduce the \textbf{Instruction Switch Dataset (ISD)}, a controlled benchmark of $3{,}000$ test cases ($300$ prompts $\times$ $10$ instruction types) where only the alignment instruction varies. Experiments on \textbf{Llama-3.1-8B} across five deterministic benchmarks (ISD, TruthfulQA, Conditional Safety, Length Control, AQI) show that CITA improves instruction sensitivity and outperforms DPO in $3/5$ metrics (TruthfulQA, Length Control, AQI); on TruthfulQA, CITA improves by $+0.054$ while DPO shows minimal change ($+0.001$). Overall, ECLIPTICA + CITA advance alignment from \emph{one-policy-per-checkpoint} toward \emph{switchable, instruction-governed} behavior suitable for modern agentic deployments.
\end{abstract}
...
On \textbf{Llama-3.1-8B}, across \textbf{five deterministic benchmarks} (including ISD and TruthfulQA),
\textbf{CITA outperforms DPO in 3/5 metrics} (TruthfulQA, Length Control, AQI);
on TruthfulQA, \textbf{CITA improves by +0.054} while \textbf{DPO changes minimally (+0.001)}.
