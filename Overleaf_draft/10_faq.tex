...
\item \textbf{CITA wins 3/5 benchmarks, DPO wins 2/5. A 60\% win rate seems marginal---why is this a significant contribution?}

The headline ``3/5 vs 2/5'' understates the key finding. When measuring \textbf{overall instruction-alignment efficiency} (mean normalized radius across all 5 benchmarks, see \cref{eq:efficiency}), \textbf{CITA achieves \colorbox{bg-green}{95.6\%}}---outperforming DPO (70.5\%) by 25.1 pp, GRPO (59.3\%) by 36.3 pp, PPO (50.4\%) by 45.2 pp, and SFT (15.9\%) by 79.7 pp. The per-benchmark breakdown:
...
%% Q7b - ATTACKS HYPERPARAMETER FAIRNESS
\item \textbf{You used Optuna for CITA but not for PPO/GRPO. Isn't this an unfair comparison that inflates CITA's results?}

 The hyperparameter strategy reflects \textbf{methodological reality}, not cherry-picking:
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Method} & \textbf{HP Source} & \textbf{Training Time} & \textbf{Optuna Feasible?} \\
\midrule
SFT & Meta playbook~\cite{dubey2024llama} & $\sim$1.5h & N/A (baseline) \\
\hline
DPO & Rafailov et al.~\cite{rafailov2023direct} & $\sim$2h & Yes (but used published) \\
\hline
PPO & TRL defaults + Ziegler~\cite{ziegler2019fine} & $\sim$17h & \textbf{No} (170h for 10 trials) \\
\hline
GRPO & DeepSeek~\cite{shao2024deepseekmath} & $\sim$12h & \textbf{No} (120h for 10 trials) \\
\hline
CITA & \textbf{Optuna (novel)} & $\sim$2h & Yes (20h for 10 trials) \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key justification}:
\begin{itemize}[leftmargin=*,itemsep=2pt]
    \item \textbf{Established methods have published HPs}: SFT, DPO, PPO, GRPO all have well-tuned hyperparameters from official papers and production deployments. We used these \textit{best-known} configurations.
    \item \textbf{CITA is novel}: No prior work exists to provide reference hyperparameters. Optuna was \textit{necessary}, not advantageous.
    \item \textbf{Compute constraint}: PPO/GRPO Optuna would require 120--170 GPU-hours---infeasible for academic compute budgets.
\end{itemize}

\vspace{4mm}

%% Q7c - ATTACKS TRAINING TIME DISPARITY
\item \textbf{PPO takes 17h, GRPO 12h, but CITA only 2h. Aren't you comparing apples to oranges?}

 Training time differences reflect \textbf{algorithmic efficiency}, not experimental unfairness:

\begin{enumerate}[leftmargin=*,itemsep=2pt]
    \item \textbf{PPO/GRPO require online generation}: Each training step generates responses, runs reward scoring, and computes policy gradients---inherently slower.

    \item \textbf{CITA/DPO are offline}: Pre-computed preference pairs enable single forward-backward passes without generation overhead.

    \item \textbf{Same hardware, same epochs}: All methods trained on identical A100-40GB with comparable epoch counts. Time difference is algorithmic, not resource allocation.

    \item \textbf{Efficiency is a feature}: If CITA achieves 95.6\% efficiency in 2h while PPO achieves 50.4\% in 17h, that's a \textit{practical advantage}, not a confound.
\end{enumerate}

\textbf{Fair comparison metric}: Results are measured on \textit{held-out benchmarks} after training completes. Training time affects reproducibility cost, not evaluation validity.

\vspace{4mm}

%% Q7d - ATTACKS BASELINE OPTIMALITY
\item \textbf{How do you know PPO/GRPO hyperparameters are optimal? Maybe they would beat CITA with better tuning.}

 We cannot prove optimality, but our baselines represent \textbf{strong, defensible configurations}:

\begin{itemize}[leftmargin=*,itemsep=2pt]
    \item \textbf{PPO}: Uses TRL library defaults + published values from InstructGPT~\cite{ouyang2022training} and Ziegler et al.~\cite{ziegler2019fine}---the same configurations used in production systems.

    \item \textbf{GRPO}: Uses DeepSeek-Math paper hyperparameters~\cite{shao2024deepseekmath}---validated at scale on mathematical reasoning.

    \item \textbf{DPO}: Uses original paper values~\cite{rafailov2023direct}---$\beta=0.1$ is the de facto standard.
\end{itemize}

\textbf{Acknowledging the limitation}: We state in Section~\ref{sec:limitations} that exhaustive HP search for all methods was computationally infeasible. However, using \textit{published best practices} is standard methodology in alignment research~\cite{ouyang2022training,bai2022training}.

\textbf{Community invitation}: All code is public. Researchers with more compute can validate whether better-tuned PPO/GRPO closes the gap.
...
\textbf{Theoretical explanation}: The difference lies in \textit{implicit} vs \textit{explicit} KL regularization:

\begin{itemize}[leftmargin=*,itemsep=2pt]
    \item \textbf{DPO's implicit KL}: In DPO, the KL constraint is baked into the loss derivation and controlled by $\beta$~\cite{rafailov2023direct}. However, $\beta$ \textit{also} controls preference strength, so KL regularization cannot be tuned independently. Research shows DPO ``tends to overlook the KL-divergence regularization, making it easy to overfit''~\cite{tang2024understanding}.

    \item \textbf{CITA's explicit KL}: CITA adds a \textit{separate} $\lambda_{\text{KL}}$ parameter that controls behavioral diversity independently from preference learning ($\beta$). This acts as a ``rehearsal mechanism'' that forces the model to maintain broad solution coverage~\cite{wang2024comprehensive}.
\end{itemize}

\textbf{Why this matters for instruction-switching}: Instruction-conditioned alignment requires maintaining \textit{multiple} behavioral modes (concise, verbose, formal, casual, etc.). DPO's mode-seeking property~\cite{goyal2024beyond} causes collapse to ONE dominant mode---explaining the 54$\times$ difference on TruthfulQA (DPO: +0.001, CITA: +0.054).
...
\midrule
KL control & $\beta$ (shared) & $\lambda_{\text{KL}}$ (separate) \\
\hline
Preference strength & $\beta$ (shared) & $\beta$ (separate) \\
\hline
Independent tuning & \textcolor{red}{\ding{55}} No & \textcolor{green}{\ding{51}} Yes \\
\hline
...
This decoupling is why CITA achieves 95.6\% instruction-alignment efficiency while DPO achieves only 70.5\%---a 25.1 pp gap explained entirely by KL architecture.
