...
\bottomrule
\end{tabular}
\caption{Instruction sensitivity ($\Delta$ = Instruct $-$ NoInstruct). CITA wins 3/5 benchmarks (TruthfulQA, Length Control, AQI); DPO wins 2/5 (ISD, Cond. Safety). However, \textbf{overall instruction-alignment efficiency}: CITA 95.6\% vs DPO 70.5\% (+25.1 pp margin).}
\label{tab:improvement_comparison}
\vspace{-4mm}
\end{table}

\begin{center}
\fbox{\textbf{Overall Instruction-Alignment Efficiency: CITA 95.6\% vs DPO 70.5\% (+25.1 pp)}}
\end{center}
...
\caption{\textbf{Instruction Alignment Efficiency}: Pentagon coverage represents overall instruction-alignment performance. \textbf{CITA achieves 95.6\%}---outperforming DPO (70.5\%) by 25.1 pp, GRPO (59.3\%) by 36.3 pp, PPO (50.4\%) by 45.2 pp, and SFT (15.9\%) by 79.7 pp.}
\label{fig:radar}
\vspace{-3mm}
\end{figure*}
